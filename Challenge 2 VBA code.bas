...
Sub stock_analysis()
'I had a lot of help from the AskBCS assistants. I just want to put that out there.
'Set dimensions
Dim total As Double
Dim i As Long
...
'get the row number of the last row with data
    ...
    If Cells(i + 1, 1).Value <> Cells(i, 1).Value Then
        ...
        Else
            ...
            Range("K" & 2 + j).Value = NumberFormat = "0.00%"
...
'take the max and min and place them in separate part in the worksheet
...
'returns one less because of header row not a factor
...
'Volume_number = WorksheetFunction.Match(Worksheet.Max(Range("L2:L" & rowCount)), Range("L2:L" & rowCount), 0)

'final ticker symbol for total vol, greatest % of increase and decrease, and average
